...
export async function sharingRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  // ── POST /api/v1/files/:id/share ──────────────────────────────────────────
  fastify.post<{ Params: { id: string } }>("/:id/share", { preHandler }, async (request, reply) => {
  ...
  // ── DELETE /api/v1/files/:id/share ────────────────────────────────────────
  fastify.delete<{ Params: { id: string } }>("/:id/share", { preHandler }, async (request, reply) => {
    const { sub: userId } = request.user as { sub: string };
    const { id: fileId } = request.params;
  ...
  fastify.get<{ Params: { id: string } }>("/:id/share", { preHandler }, async (request, reply) => {
    const { sub: userId } = request.user as { sub: string };
    const { id: fileId } = request.params;
