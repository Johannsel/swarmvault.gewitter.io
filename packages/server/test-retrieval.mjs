...
const gfDiv = (a, b) => { if (!b) throw new Error("GF div0"); if (!a) return 0; return GF_EXP[((GF_LOG[a] - GF_LOG[b]) + 255) % 255]; };
function makeVandermonde(rows, cols) {
  return Array.from({ length: rows }, (_, r) =>
    Array.from({ length: cols }, (_, c) => r === 0 ? 1 : gfMul(GF_EXP[r-1], c === 0 ? 1 : GF_EXP[((r-1)*c)%255]))
  );
}
function matMul(a, b) {
  return Array.from({ length: a.length }, (_, r) =>
    Array.from({ length: b[0].length }, (_, c) =>
      a[r].reduce((s, _, k) => s ^ gfMul(a[r][k], b[k][c]), 0)
    )
  );
}
function invertMatrix(m) {
  ...
  for (let col = 0; col < n; col++) {
    ...
    for (let c = 0; c < n; c++) { work[col][c] = gfDiv(work[col][c], scale); inv[col][c] = gfDiv(inv[col][c], scale); }
    for (let r = 0; r < n; r++) {
      if (r === col) continue;
      const f = work[r][col];
      for (let c = 0; c < n; c++) { work[r][c] ^= gfMul(f, work[col][c]); inv[r][c] ^= gfMul(f, inv[col][c]); }
    }
  }
  return inv;
}
function encodeFile(data, dataShards, parityShards) {
  ...
  const padded = Buffer.alloc(shardSize * dataShards); data.copy(padded);
  ...
  for (let p = 0; p < parityShards; p++) {
    ...
    for (let b = 0; b < shardSize; b++) {
      let v = 0; for (let d = 0; d < dataShards; d++) v ^= gfMul(row[d], shards[d][b]);
      pShard[b] = v;
    }
    shards.push(pShard);
  }
...
function decodeFile(shards, dataShards, parityShards, originalSize) {
  ...
  const recovered = Array.from({ length: dataShards }, (_, d) => {
    const row = decodeMatrix[d]; const res = Buffer.alloc(shardSize);
    for (let b = 0; b < shardSize; b++) {
      let v = 0; for (let k = 0; k < dataShards; k++) v ^= gfMul(row[k], shards[used[k]][b]);
      res[b] = v;
    }
    return res;
  });
...
function serializeEncryptedChunk({ ciphertext, iv, authTag }) { return Buffer.concat([iv, authTag, ciphertext]); }
...
function decryptChunk({ ciphertext, iv, authTag }, key) {
  const d = createDecipheriv("aes-256-gcm", key, iv); d.setAuthTag(authTag);
...
const NODE_ID = nodeData.node.id, RELAY_TOKEN = nodeData.node.relayToken;
...
await new Promise((resolve, reject) => {
  ...
  ws.on("message", raw => {
    ...
    if (msg.type === "auth_ack") { console.log("WS auth_ack ✓"); resolve(); }
...
const { file, shardAssignment } = await fetch(`${SERVER}/api/v1/files`, {
  method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${TOKEN}` },
  body: JSON.stringify({
    name: "test-retrieval.bin", path: "/test-retrieval.bin",
    sizeBytes: originalData.length, tier: "swarm",
    contentHash: sha256(originalData), totalShards: DATA_SHARDS, parityShards: PARITY_SHARDS,
...
for (let i = 0; i < TOTAL_SHARDS; i++) {
  ...
  const res = await fetch(`${SERVER}/api/v1/chunks`, {
    ...
    headers: {
      ...
      "X-File-Id": file.id, "X-Shard-Index": String(i),
      "X-Chunk-Hash": chunkHash, "X-Is-Data": String(i < DATA_SHARDS), "X-Node-Id": NODE_ID,
    },
    body: serialized,
  });
