...
const api = {
  ...
  // Node management
  registerNode: (opts: { displayName: string; tier: "vault" | "swarm"; pledgedBytes: number }) =>
    ipcRenderer.invoke("node:register", opts),
  ...
  // File operations
  listFiles: (params: { path?: string; status?: string }) =>
    ipcRenderer.invoke("files:list", params),
  downloadFileToSync: (fileId: string, filePath: string) =>
    ipcRenderer.invoke("files:downloadToSync", { fileId, filePath }) as Promise<{ ok: boolean }>,
  deleteFileFromVault: (fileId: string) =>
    ipcRenderer.invoke("files:deleteFromVault", { fileId }) as Promise<{ ok: boolean }>,
  ...
  restoreFile: (fileId: string) =>
    ipcRenderer.invoke("files:restore", { fileId }) as Promise<{ file: unknown }>,
  deleteFilePermanent: (fileId: string) =>
    ipcRenderer.invoke("files:deletePermanent", { fileId }) as Promise<{ ok: boolean }>,

  // Selective sync
  getSyncedFolders: () => ipcRenderer.invoke("sync:getFolders") as Promise<string[]>,
  setSyncedFolders: (folders: string[]) =>
    ipcRenderer.invoke("sync:setFolders", { folders }) as Promise<{ ok: boolean }>,

  // File sharing
  shareFile: (fileId: string) =>
    ipcRenderer.invoke("files:share", { fileId }) as Promise<{ token: string; shareUrl: string; expiresAt: string }>,
  getShareStatus: (fileId: string) =>
    ipcRenderer.invoke("files:shareStatus", { fileId }) as Promise<{ token: string; shareUrl: string; expiresAt: string } | null>,
  unshareFile: (fileId: string) =>
    ipcRenderer.invoke("files:unshare", { fileId }) as Promise<{ ok: boolean }>,
