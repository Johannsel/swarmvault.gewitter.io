...
export default function FileManager() {
  ...
  return (
    <div className="p-6 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-lg overflow-hidden border border-slate-700 text-xs">
            <button
              ...
              className={`px-3 py-1.5 transition-colors ${tab === "files" ? "bg-violet-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
            >
              Files
            </button>
            <button
              onClick={() => setTab("trash")}
              className={`px-3 py-1.5 transition-colors flex items-center gap-1 ${tab === "trash" ? "bg-violet-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
            >
              <Trash2 size={11} /> Trash
            </button>
          </div>
          <button
            onClick={tab === "files" ? load : loadTrash}
      ...
      {actionResult && (
        <div className={`text-xs px-4 py-2 rounded-lg ${actionResult.ok ? "bg-emerald-900/30 text-emerald-400" : "bg-red-900/30 text-red-400"}`}>
          {actionResult.msg}
        </div>
      )}
      {shareError && (
        <div className="text-xs px-4 py-2 rounded-lg bg-red-900/30 text-red-400">✗ {shareError}</div>
      )}
      ...
      {quota?.overQuota && (
        <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-amber-900/30 border border-amber-700/40 text-amber-300">
          ...
          <div className="text-xs leading-relaxed">
            <span className="font-medium">Storage quota reached.</span>{" "}
            {formatBytes(quota.usedBytes)} used of {formatBytes(quota.quotaBytes)}.{" "}
            Your existing files are safe — but new uploads are paused until you earn more credits by
            keeping your node online, or delete files to free up space.
          </div>
        </div>
      )}
      ...
      {tab === "files" && (
        loading ? (
          ...
          <div className="flex items-center justify-center h-48 text-slate-500 text-sm">
            No files yet. Add files to your sync folder to get started.
          </div>
        ) : (
                ...
                <div key={file.id} className="bg-slate-800 rounded-lg overflow-hidden">
                  ...
                  {activeShare && (
                    <div className="flex items-center gap-2 px-4 py-2 bg-sky-950/40 border-t border-sky-900/30 text-xs">
                      ...
                      <button
                        onClick={() => copyShareUrl(file.id, activeShare.shareUrl)}
                        className="flex items-center gap-1 px-2 py-1 rounded bg-sky-800 hover:bg-sky-700 text-sky-200 transition-colors flex-shrink-0"
                      >
                        <Copy size={11} />
            ...
            {trashFiles.map((file) => {
              ...
              return (
                <div key={file.id} className="bg-slate-800 rounded-lg flex items-center gap-3 px-4 py-3">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${days <= 3 ? "bg-red-900/40 text-red-400" : "bg-slate-700 text-slate-400"}`}>
                    {days}d left
                  </span>
